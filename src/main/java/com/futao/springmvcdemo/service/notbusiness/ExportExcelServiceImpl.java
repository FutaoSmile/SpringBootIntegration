...
import com.futao.springmvcdemo.utils.CommonUtils;
...
//        headTitle.createCell(0).setCellValue("重点项目计划表");
        ...
        try {
            ...
            throw ApplicationException.le(ErrorMessage.ApplicationException.EXPORT_EXCEL_FAIL);
        } finally {
            if (outputStream != null) {
                try {
                    ...
                    throw ApplicationException.le(ErrorMessage.ApplicationException.CLOSE_OUTPUT_STREAM_FAIL);
                }
            }
            try {
                workbook.close();
        ...
        objs.forEach(obj -> {
                    ...
                    for (int i = 0; i < methods.length; i++) {
                        try {
                            ...
                            LOGGER.error("执行方法异常");
                            throw ApplicationException.le(ErrorMessage.ApplicationException.INVOKE_METHOD_FAIL);
                        }
                    }
